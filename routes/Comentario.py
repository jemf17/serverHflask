...
async def add_coment(fecha: str = Body(...), desc: str = Body(...), user: UUID=Body(...), obra: UUID=Body(...), capitulo: int = Body(...)):
    try:
        comentario = ComentUser(fecha,desc, user)
        afect_rows = ComentarioModel.add_coment(obra, comentario, capitulo)
        if afect_rows == 0:
...
@mainComent.get('/obra/{obra}')
async def get_coment_obra(obra: UUID = Path(...)):
    try:
        return {'comentarios':ComentarioModel().get_all_coments_by_obra(obra)}
    except Exception as ex:
        return {'message':str(ex),'status':500}
@mainComent.get('/obra/{obra}/{capitulo}')
async def get_coment_obra_capitulo(obra: UUID = Path(...), capitulo: int = Path(...)):
    try:
        return {'comentarios':ComentarioModel().get_all_coments_by_capi(obra, capitulo)}
    except Exception as ex:
        return {'message':str(ex),'status':500}